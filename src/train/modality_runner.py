...
MODALITY_METHODS = {"mod_seq_ft", "mod_seq_kd", "prototype_replay", "cmcrd_ours"}


def run_modality_experiment(config_path: str | Path, method: str) -> Path:
    ...
    for stage_name in stage_order:
        ...
        if method in {"mod_seq_kd", "cmcrd_ours"}:
...
def _output_dir(config: dict) -> Path:
    train_cfg = config.get("train", {})
    return resolve_path(train_cfg.get("output_dir", "outputs"), PROJECT_ROOT)
